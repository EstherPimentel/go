...
// PatchName
func (h *productHandler) PatchName() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		var productName domain.ProductName
		err := ctx.ShouldBindJSON(&productName)
		if err != nil || productName.Name == "" {
			ctx.JSON(400, gin.H{"error": "invalid product name"})
			return
		}
		...
		p, err := h.s.PatchName(id, productName.Name)
		if err != nil {
			ctx.JSON(400, gin.H{"error": err.Error()})
			return
		}
...
func (h *productHandler) Delete() gin.HandlerFunc {
	...
	return func(ctx *gin.Context) {
		...
		ctx.JSON(200, gin.H{"data": fmt.Sprintf("product with id %d deleted", id)})
